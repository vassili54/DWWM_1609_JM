...
class personal_cv_resume_Post_Related {
	/**
	...
	public function __construct() {
		...
		if( !is_admin()  )
		{
			add_action( 'personal_cv_resume_site_content_type', array( $this,'site_loop_heading' ), 20 ); 

			add_action( 'personal_cv_resume_site_content_type', array( $this,'site_content_type' ), 30 ); 
		}
		
		add_action( 'personal_cv_resume_posts_blog_media', array( $this, 'render_thumbnail' ) ); 
		
		
		add_action( 'personal_cv_resume_loop_navigation', array( $this,'site_loop_navigation' ) );
		...
		add_action( 'personal_cv_resume_single_post_navigation', array( $this,'single_post_navigation' ),20 );
    ...
    function site_content_type( ){
		...
		$type = apply_filters( 'personal_cv_resume_content_type_filter', personal_cv_resume_get_option( 'blog_loop_content_type' ) );
	...
	*/
	public function content_read_more_link( $more  ) {
		...
		return sprintf( '<div class="more-link">
             <a href="%1$s" class="link-btn theme-btn"><span>%2$s </span><i class="fa fa-caret-right"></i></a>
        </div>',
            esc_url( get_permalink( get_the_ID() ) ),
		    esc_html( personal_cv_resume_get_option( 'read_more_text' ) )
        );
	...
	*/
	public function excerpt_read_more_link( $more ) {
		if ( is_admin() ) return $more;
		$text = !empty(personal_cv_resume_get_option( 'read_more_text' )) ? personal_cv_resume_get_option( 'read_more_text' ) : esc_html__('Read More', 'personal-cv-resume');
		if ( ! is_single() ) {
	...
	function site_loop_navigation( $type = '' ) {
		...
		'type' => 'list',
		'mid_size' => 2,
		'prev_text' => esc_html__( 'Previous', 'personal-cv-resume' ),
		'next_text' => esc_html__( 'Next', 'personal-cv-resume' ),
		'screen_reader_text' => esc_html__( '&nbsp;', 'personal-cv-resume' ),
		) );
		echo '</div>';
	...
	function single_post_navigation( ) {
		...
		if( $prevPost ) :
			$html .= '<div class="col-md-6 col-sm-6"><div class="row align-items-center">';
				...
				$html .='<div class="col-md-9"><h5>'.esc_html__('Previous Article','personal-cv-resume').'</h5>';
		...
		if( $nextPost ) : 
			$html .= '<div class="col-md-6 col-sm-6"><div class="row align-items-center">';
				...
				$html .='<div class="col-md-9 ml-auto align-right"><h5>'.esc_html__('Next Article','personal-cv-resume').'</h5>';
		...
		$html  	 = apply_filters( 'personal_cv_resum_single_post_navigation_filter', $html );
	...
	public function get_gallery_thumbnail(){
		...
		if( has_block('gallery', $post->post_content) ): 
			...
			if ( !is_singular() ){
			$html  .= '<a href="'.esc_url( get_permalink(get_the_ID()) ).'" class="article-link">';
			$html .= '<i class="dashicons dashicons-arrow-right-alt"></i></a>';
			}
			$html .= '</div>';
		...
		$html =  apply_filters( 'personal_cv_resume_gallery_thumbnail', $html );
	...
	public function get_audio_thumbnail(){
		...
		{	 $i = 0;
			...
			foreach ( $audio as $audio_html ) : $i++;
				...
				{
					$html .= '<span class="post-format-icon"><i class="dashicons dashicons-format-audio"></i></span>';
		...
		$html =  apply_filters( 'personal_cv_resume_audio_thumbnail', $html );
	...
	public function get_video_thumbnail(){
		...
		if ( ! empty( $video ) ) {	
			$html = '<div class="img-box"><span class="post-format-icon"><i class="dashicons dashicons-format-video"></i></span>';
			$i = 0;
		...
		$html =  apply_filters( 'personal_cv_resume_video_thumbnail', $html );
	...
	public function get_image_thumbnail(){
		...
		if ( has_post_thumbnail() ) :
			...
			$html = '<div class="img-box"><span class="post-format-icon"><i class="dashicons dashicons-format-image"></i></span>';
			$post_thumbnail_id  = get_post_thumbnail_id( get_the_ID() );
			$post_thumbnail_url = wp_get_attachment_url( $post_thumbnail_id );
			
			if ( is_singular() ){
				$html  .=  '<a href="'.esc_url( $post_thumbnail_url ).'" class="thickbox gallery-image">';
				$html .= '<div class="img-wrp">'.get_the_post_thumbnail( get_the_ID(), 'full' ).'</div>';
			} else{
				$html .= '<div class="img-wrp">'.get_the_post_thumbnail( get_the_ID(), 'full' ).'</div>';
				$html  .= '<a href="'.esc_url( get_permalink(get_the_ID()) ).'" class="gallery-image article-link">';
		...
		$html =  apply_filters( 'personal_cv_resume_image_thumbnail', $html );
...
$personal_cv_resume_post_related = new personal_cv_resume_Post_Related();