...
function validateForm() {
    ...
    if (firstName.trim() !== "" && age !== "" && parseInt(age) > 0) {
        ...
        const message = parseInt(age) >= 18 ? 'La personne est <span class="blue-text">majeure</span>.' : 'La personne est <span class="blue-text">mineure</span>.';
        ...
        if (parseInt(age) < retirementAge) {
            retirementMessage = `Il vous reste <span class="blue-text">${yearsDifference}</span> année(s) avant la retraite.`;
        } else if (parseInt(age) > retirementAge) {
            retirementMessage = `Vous êtes à la retraite depuis <span class="blue-text">${yearsDifference}</span> année(s).`;
        } else {
        ...
        resultDIV.innerHTML = `
            <p>Bonjour <span class="blue-text">${firstName}</span>, votre âge est : <span class="blue-text">${age}</span>.</p>
            <p>${message}</p>
